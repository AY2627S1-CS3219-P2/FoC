...
//   OTP step needs (F1.1.2.3-F1.1.2.7).
// Author review: PENDING — <reviewer to complete>

/**
 * SHAPE, NOT CONTRACT. `user-service` does not exist yet and its owner defines
 * the real identity payload (root AGENTS.md §1). The FIELDS here are taken
 * from the backlog rather than invented: F1.4.1 says a user can view their
 * account identifier, registered email, username and contact information, and
 * F1.5 makes the role STUDENT or ADMIN.
 */
export interface Session {
  ...
  username: string;
  /** F1.4.1. F1.4.3 makes it unchangeable. */
  email: string;
  /** F1.4.1 "contact information". F1.4.2 makes it updatable. */
  contact: string;
