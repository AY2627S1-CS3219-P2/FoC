...
**Four of the D2 rows below already have answers in a draft
`user-service/AGENTS.md` that Zi Yang circulated on 2026-09-19** — the header
names (`X-User-ID`, `X-User-Role`), the signing algorithm (RS256 with a JWKS
endpoint), the token lifetimes (15 minutes / 7 days) and the auth endpoint
paths. They stay listed as open because that file is **not committed to any
branch**, and §1 counts a decision as made only once it is somewhere the team
can point at. Merging that file closes them.
