...
**A decision counts as recorded** only if it is in `ai/decisions.md`, in a
merged `api/openapi.yaml`, or in code already on `main`. "We decided X" in a
prompt is not a record — ask for it to be written down first, then implement it.
...
1. Append a row to `## Entries` in `ai/usage-log.md` for every session where you
   change files **or where this section made you hand something back**.
...
- Don't commit unless the human asks. Don't push to `main`.
